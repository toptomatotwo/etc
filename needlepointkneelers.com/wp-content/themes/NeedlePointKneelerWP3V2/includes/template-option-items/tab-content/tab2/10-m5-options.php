	...
	<?php // first column ?>
 	...
 	<div class="metabox-holder">
		...
		<div class="postbox">
		<h3 class="postbox-header"><?php _e("Magazine #5 Template Options", 'kubrick'); ?></h3>
			<div class="inside">
			<h4>Featured Top Content Area</h4>

				<p><?php _e("Number of characters to limit the post content", 'kubrick'); ?>:<br />
				<input type="text" name="<?php echo $settings; ?>[featured5_top_chars]" value="<?php echo tt_option('featured5_top_chars'); ?>" size="3" /> characters</p>
				<p><?php _e("Number of posts to show", 'kubrick'); ?>:<br />
				<input type="text" name="<?php echo $settings; ?>[featured5_top_num]" value="<?php echo tt_option('featured5_top_num'); ?>" size="3" /></p>
				
				<p><?php _e("Thumbnail dimensions", 'kubrick'); ?> (<?php _e("Width x Height", 'kubrick'); ?>)<br />
				<input type="text" name="<?php echo $settings; ?>[featured5_top_thumb_width]" value="<?php echo tt_option('featured5_top_thumb_width'); ?>" size="3" /> px wide by <input type="text" name="<?php echo $settings; ?>[featured5_top_thumb_height]" value="<?php echo tt_option('featured5_top_thumb_height'); ?>" size="3" /> px high</p>
				
				<h4><?php _e("Featured Top Left", 'kubrick'); ?></h4>
				<p><?php _e("Select which category you want displayed", 'kubrick'); ?>:<br />
    			<?php wp_dropdown_categories(array('selected' => tt_option('featured5_top_left'), 'name' => $settings.'[featured5_top_left]', 'orderby' => 'Name' , 'hierarchical' => 1, 'hide_empty' => '0', 'show_count' => 1 )); ?></p>
				
				<h4><?php _e("Featured Top Right", 'kubrick'); ?></h4>
				<p><?php _e("Select which category you want displayed", 'kubrick'); ?>:<br />
    			<?php wp_dropdown_categories(array('selected' => tt_option('featured5_top_right'), 'name' => $settings.'[featured5_top_right]', 'orderby' => 'Name' , 'hierarchical' => 1, 'hide_empty' => '0', 'show_count' => 1 )); ?></p>
				
				<h4><?php _e("Featured Bottom Left", 'kubrick'); ?></h4>
				<p><?php _e("Select which category you want displayed", 'kubrick'); ?>:<br />
    			<?php wp_dropdown_categories(array('selected' => tt_option('featured5_bottom_left'), 'name' => $settings.'[featured5_bottom_left]', 'orderby' => 'Name' , 'hierarchical' => 1, 'hide_empty' => '0', 'show_count' => 1 )); ?></p>
				
				<h4><?php _e("Featured Bottom Right", 'kubrick'); ?></h4>
				<p><?php _e("Select which category you want displayed", 'kubrick'); ?>:<br />
    			<?php wp_dropdown_categories(array('selected' => tt_option('featured5_bottom_right'), 'name' => $settings.'[featured5_bottom_right]', 'orderby' => 'Name' , 'hierarchical' => 1, 'hide_empty' => '0', 'show_count' => 1 )); ?></p>
				
				<p><?php _e("Do you want make the content blocks transparent?", 'kubrick'); ?><br />
				<select name="<?php echo $settings; ?>[m5_block_transparent]">
					<option style="padding-right:10px;" value="Yes" <?php selected('Yes', tt_option('m5_block_transparent')); ?>>Yes</option>
					<option style="padding-right:10px;" value="No" <?php selected('No', tt_option('m5_block_transparent')); ?>>No</option>
				</select></p>
			</div>
		</div>
		...
		<img style="margin:0 0 0 25px;" src="<?php bloginfo('template_url'); ?>/includes/template-option-items/mag5.jpg" />		
	</div>
	...
	<div class="metabox-holder">
        <div class="postbox">
		<h3 class="postbox-header"><?php _e("Magazine #5 Template Help", 'kubrick'); ?></h3>
		<div class="inside">
		<p>
		The Magazine #5 template creates a page with two post content blocks positioned above two more post content block. Each block can contain posts from different categories.
		</p>
		<h4>Left & Right Post Content Areas</h4>
		<p>
		<ol>
