...
{	global $post;
	...
	$menu = &$args['menu'];
	$alt = &$args['alt'];
	$alt_menu = &$args['alt_menu'];	$class = &$args['class'];
	if (function_exists('get_nav_menu_locations') && $menu != null && is_string($menu)) { // theme location
		...
		if ($location) {
			if ($alt_menu == 1) {
			$menus = get_terms( 'nav_menu', array( 'hide_empty' => false ) );$mmenu = ( $menus[theme_get_meta_option($post->ID, 'theme_nav_menu_items')]);$mmenu = $mmenu->slug;
			$menu = wp_get_nav_menu_object($mmenu);} 
			else 
