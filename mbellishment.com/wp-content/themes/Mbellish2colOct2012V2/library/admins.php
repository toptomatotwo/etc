...
?>
	<p class="submit">
		<input name="Submit" type="submit" class="button-primary" value="<?php echo esc_attr(__('Save All Changes', THEME_NS)) ?>" />
		<input name="Reset" type="submit" class="button-secondary" value="<?php echo esc_attr(__('Reset All To Default', THEME_NS)) ?>" />

	</p>
<?php
			echo '<h3 style="background-color:#ccc;padding:5px;margin:0;">'.$name.'</h3>'."\n";
			if ($desc) {
				echo "\n".'<p class="description"style="background-color:#ccc;color:#444;padding:5px;margin:0px;">'.$desc.'</p>'."\n";
			}
		} else {
			if (!$in_form_table) {
				echo '<table class="form-table" style="margin-top:0px;font-size:12px;border:1px solid #ccc;background-color:#eee;">'."\n";
				$in_form_table = true;
			}
			echo '<tr valign="top">'."\n";
...
function theme_save_post($post_id) {
	...
	if ( 'page' == $_POST['post_type']) {
		...
		if ( $post_type == $_POST['post_type'] && $post_type != 'page') {
