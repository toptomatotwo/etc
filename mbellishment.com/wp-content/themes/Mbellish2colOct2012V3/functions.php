<?php
define('THEME_NAME',"Mbellish2colOct2012V3");
global $wp_version;
define('WP_VERSION', $wp_version);
define('THEME_OPTIONS','3.1.215');
define('FRAMEWORK_VERSION1','3.1.0.46558W, 3.1.0.48375W');
define('FRAMEWORK_VERSION2','3.1.0.46558M, 3.1.0.48375M');
...
function tt_shortcode_option( $atts, $content = null)	{
	...
	$new_option = theme_get_option($id);

	$content = ob_start();echo '<pre>'.$id.'<br />';print_r($new_option);echo '</pre>';$content = ob_get_clean();
	
	return $content;
}
add_shortcode('theme_option', 'tt_shortcode_option');
...
if (WP_VERSION < 3.0){
	require_once(get_template_directory() . '/library/legacy.php');
}
...
if (file_exists(get_template_directory().'/includes/template_functions.php'))
include(get_template_directory()."/includes/template_functions.php");
if (file_exists(get_template_directory().'/includes/tt_functions.php'))
include(get_template_directory()."/includes/tt_functions.php");
if (file_exists(get_template_directory().'/includes/core-additions.php'))
include(get_template_directory()."/includes/core-additions.php");

if (theme_get_option('read_more_change') == 1) {$read_more_gtc = theme_get_option('read_more_text');$read_more = theme_get_option('read_more_text'); } else {$read_more_gtc = 'Read more...';$read_more = 'Continue reading &rarr;';}
if ((theme_get_option('remove_elipse')) == 1) {$elipse = '';} else {$elipse = '&hellip;';}
// end
...
if(is_admin()){
  ...
  {
    global $theme_name;
     add_theme_page('Theme Options' , 'Theme Options', 'administrator', basename(__FILE__), 'ttpanel_admin');
  }	


    add_action('admin_menu', 'manage_admin_menu');
    include 'admin/ttpanel/ttpanel.php'; 
	if (WP_VERSION >= 3.0) {
...
function theme_get_option($name){
	global $theme_default_options, $options, $result;
...
if (!function_exists('theme_get_content')){
	function theme_get_content($args = array()) {
		...
		ob_start();tt_content_before();echo apply_filters('the_content', $content);tt_content_after();$content = ob_get_clean();
...
if (!function_exists('theme_get_excerpt')){
	function theme_get_excerpt($args = array()) {
		global $post,$read_more,$elipse,$read_more_gtc;
		$read_more_choice = theme_get_option( 'read_more_choice' );

	switch ( $read_more_choice ) {
		case 'image':
		ob_start(); ?>
        <div class='read-more-image'><a class='more-link' href='<?php the_permalink(); ?>' ><img class='read-more-img' src="<?php echo theme_get_option('read_more_img'); ?>" /></a></div>
		<?php $more_tag = ob_get_clean();
        ...
        <span class="art-button-wrapper">
            ...
            <input class="art-button" type="submit" name="<?php echo $read_more; ?>" value="<?php echo $read_more; ?>" />
        </span>
		</form>
		</div>
		<?php $more_tag = ob_get_clean();
		...
		case 'none':
		$more_tag = theme_get_array_value($args, 'more_tag', __($read_more, THEME_NS));
		...
		if ($auto && has_excerpt($post->ID)) {
			...
			// hack for badly written plugins
		    ob_start();tt_excerpt_before();echo apply_filters('the_content', $excerpt);$excerpt = ob_get_clean();
			global $multipage;
