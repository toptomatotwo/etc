...
<div class="postbox ">
    ...
    <p>&nbsp;&nbsp;</p>
       ...
       <?php if(!empty($template)){
		   $row = $wpdb->get_row("select * from $lpObj->tablename where id=$template");
		  // print_r($row);
	   }?>
        	<p><input type="text" name="lp-title" id="lp-title" value="<?php echo isset($row->title)?$row->title:'Terms of Use';?>" /></p>
            <p>
            <div id="poststuff">
                <div id="<?php echo user_can_richedit() ? 'postdivrich' : 'postdiv'; ?>" >
                	<?php 
					$content = isset($row->content)?$row->content:$terms;
					the_editor(stripslashes(html_entity_decode($content))); ?>
                </div>
	...
	$result = $wpdb->get_results("select * from $lpObj->tablename");
		foreach($result as $ras){
			?>
            <li><span id="legalpages<?php echo $ras->id;?>"><a href="<?php echo $baseurl;?>?page=<?php echo $page;?>&template=<?php echo $ras->id;?>"><?php echo $ras->title;?> &raquo;</a></span></li>
            <?php }?>
